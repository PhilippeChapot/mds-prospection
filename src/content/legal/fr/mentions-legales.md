# Mentions légales

_Dernière mise à jour : 4 mai 2026_

## Éditeur du site

Le site **mediadays.solutions** est édité par :

**Editions HF — Podcast & RadioHouse**
8 rue Fernand Delmas
19100 Brive-la-Gaillarde
France

**Contact :** [philippe@mediadays.solutions](mailto:philippe@mediadays.solutions)

**SIRET / RCS :** _à renseigner_

**Directeur de publication :** Philippe Chapot

## Hébergement

Le site est hébergé par :

- **Vercel Inc.** — 340 S Lemon Ave #4133, Walnut, CA 91789, USA — pour le rendu et le serveur web. Région d'hébergement : `cdg1` (Paris, Union européenne).
- **Supabase** — base de données et authentification, hébergées dans la région `eu-west-3` (Frankfurt, Allemagne).

## Propriété intellectuelle

L'ensemble des contenus présents sur le site (textes, logos, images, vidéos, etc.) est protégé par le droit de la propriété intellectuelle et appartient à Editions HF ou à ses partenaires. Toute reproduction, représentation ou diffusion, totale ou partielle, sans autorisation écrite préalable, est interdite.

Les marques **MediaDays Solutions**, **Paris Radio Show** et leurs logos respectifs sont des marques déposées d'Editions HF.

## Crédits

- Conception et développement : Editions HF.
- Identité visuelle : charte MediaDays Solutions 2026.

## Contact

Pour toute question relative au site ou à son contenu, vous pouvez écrire à [philippe@mediadays.solutions](mailto:philippe@mediadays.solutions).
