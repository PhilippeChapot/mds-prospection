 ...
 *   - getProspectForExposant (helper queries)
...
function mockAll(state: State) {
  vi.doMock('@/lib/espace-exposant/jwt', () => ({
    ESPACE_EXPOSANT_SESSION_COOKIE: 'espace_exposant_session',
  ...
  vi.doMock('./queries', async () => {
    ...
    return {
      ...actual,
      getProspectForExposant: vi.fn().mockResolvedValue(state.prospect ?? null),
    };
  });
...
describe('createSupplementaryCheckoutSession (P6.x.1b)', () => {
  beforeEach(() => {
    ...
    cookieValue.espace_exposant_session = 'valid-jwt-token';
  ...
  afterEach(() => {
    ...
    cookieValue.espace_exposant_session = undefined;
    vi.restoreAllMocks();
    vi.resetModules();
  });
