 ...
 * (espace-exposant JWT cookie). Pour les preferences email, voir
...
const LOG_PREFIX = '[espace-exposant/profile-actions]';
...
export async function updateMyContactProfileAction(
  ...
  revalidatePath(`/${parsed.data.locale}/espace-exposant/dashboard/profil`);
  return { ok: true };
}
