...
/**
 * P3.1 — Server actions pour les ressources partenaire (guide markdown bilingue).
 *
 * Actions :
...
export async function listResourcesAction(): Promise<ActionResult<PartnerResourceRow[]>> {
  ...
  return { ok: true, data: (data ?? []) as PartnerResourceRow[] };
}

// ---------------------------------------------------------------------------
// getPublishedResourcesAction (anon / espace partenaire)
// ---------------------------------------------------------------------------
...
export async function upsertResourceAction(
  ...
  let before: PartnerResourceRow | null = null;
  if (data.id) {
    ...
    before = existing as PartnerResourceRow;
  }
  ...
  revalidatePath('/admin/partner-resources');
  revalidatePath('/fr/espace-partenaire/dashboard/ressources');
  revalidatePath('/en/espace-partenaire/dashboard/ressources');
  revalidatePath(`/fr/espace-partenaire/dashboard/ressources/${data.slug}`);
  revalidatePath(`/en/espace-partenaire/dashboard/ressources/${data.slug}`);
