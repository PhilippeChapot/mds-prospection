/**
 * Template magic-link Espace Exposant — P5.x.2.
 *
 * Email transactionnel envoye apres POST /api/espace-exposant/request-magic-link
 * pour permettre a l'exposant d'acceder a son dashboard sans password.
 *
...
export interface EspaceExposantMagicLinkParams {
  firstName: string;
  /** URL absolue de la forme `${SITE_URL}/{locale}/espace-exposant/login?token=...`. */
  magicLinkUrl: string;
  /** URL de la page demande (utile pour le hint "redemander un lien"). */
  requestPageUrl: string;
}

export interface EspaceExposantMagicLinkTemplate {
  subject: string;
  html: string;
  text: string;
}
...
function renderFr(p: EspaceExposantMagicLinkParams): EspaceExposantMagicLinkTemplate {
  const subject = `Votre lien d'accès Espace Exposant MediaDays Solutions 2026`;
    ...
    <div style="${BASE_STYLES}">
      <div style="max-width: 560px; margin: 0 auto; background: #fff; border: 1px solid #e0e4ee; border-radius: 12px; padding: 32px;">
        ...
        <p style="margin: 0 0 24px; line-height: 1.55;">
          Cliquez sur le bouton ci-dessous pour accéder à votre Espace Exposant MediaDays Solutions 2026 :
        </p>

        <p style="margin: 0 0 24px;">
          <a href="${escapeAttr(p.magicLinkUrl)}" style="display: inline-block; padding: 14px 28px; background: #e6007e; color: #fff; text-decoration: none; border-radius: 8px; font-weight: 700;">Accéder à mon Espace Exposant</a>
        </p>
  ...
  const text = [
    `Bonjour ${p.firstName},`,
    ``,
    `Cliquez sur le lien ci-dessous pour acceder a votre Espace Exposant MediaDays Solutions 2026 :`,
    ``,
    p.magicLinkUrl,
    ``,
...
function renderEn(p: EspaceExposantMagicLinkParams): EspaceExposantMagicLinkTemplate {
  const subject = `Your MediaDays Solutions 2026 Exhibitor Portal access link`;
  ...
  const html = `
    <div style="${BASE_STYLES}">
      <div style="max-width: 560px; margin: 0 auto; background: #fff; border: 1px solid #e0e4ee; border-radius: 12px; padding: 32px;">
        ...
        <p style="margin: 0 0 24px; line-height: 1.55;">
          Click the button below to access your MediaDays Solutions 2026 Exhibitor Portal:
        </p>

        <p style="margin: 0 0 24px;">
          <a href="${escapeAttr(p.magicLinkUrl)}" style="display: inline-block; padding: 14px 28px; background: #e6007e; color: #fff; text-decoration: none; border-radius: 8px; font-weight: 700;">Access my Exhibitor Portal</a>
        </p>
  ...
  const text = [
    `Hi ${p.firstName},`,
    ``,
    `Click the link below to access your MediaDays Solutions 2026 Exhibitor Portal:`,
    ``,
    p.magicLinkUrl,
    ``,
