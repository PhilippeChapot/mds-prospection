/**
 * Documents + Kit communication pour l'Espace Partenaire V1.1 — P5.x.10.
 *
 * Pure functions : pas d'IO. Recoivent le `prospect` (lu cote
 * session.ts) et le `locale`, retournent les URLs / contenus a afficher.
 *
 * Guide partenaire PDF + plan de salle PDF : `null` tant que les assets
...
export interface DocumentLinks {
  /** PDF guide partenaire. null tant que le PDF n'est pas livre. */
...
/**
 * Construit le bloc Documents du dashboard partenaire.
 *
 * Env vars optionnelles :
 *   - EXHIBITOR_GUIDE_PDF_URL : URL du guide partenaire PDF
...
/**
 * Construit le Kit communication a afficher cote Espace Partenaire.
 *
...
export function getEmailSignatureHtml(locale: 'fr' | 'en', isPrsExhibitor: boolean): string {
  ...
  // P5.x.10.ter : double lien footer (partner + visitor) avec
  // target=_blank + rel=noopener (anti-tabnabbing) sur chaque ancre.
  // Liens hardcodes : mediadays.solutions = B2B partenaires, mediadays.net
  // = B2C visiteurs (independants de NEXT_PUBLIC_APP_URL qui pointe
  // sur le sous-domaine du repo courant).
  const exhibitorLabel = locale === 'en' ? 'Partner info:' : 'Infos partenaires :';
