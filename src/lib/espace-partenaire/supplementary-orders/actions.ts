 ...
 *
 * Flow :
 *   1. Lit le cookie session espace-partenaire → prospect_id
...
import { getProspectForPartenaire } from './queries';
 ...
 * Le cookie session espace-partenaire détermine le prospect (pas de prospect_id
...
export async function createSupplementaryCheckoutSession(input: unknown): Promise<CheckoutResult> {
  ...
  // 3. Éligibilité
  const prospect = await getProspectForPartenaire(prospectId);
  ...
  try {
    session = await stripe.checkout.sessions.create({
      ...
      success_url: `${appUrl}/fr/espace-partenaire/dashboard/commandes/${order.id}?paid=1`,
      cancel_url: `${appUrl}/fr/espace-partenaire/dashboard/commander`,
      metadata: {
