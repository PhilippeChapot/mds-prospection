 ...
 * - getProspectForPartenaire(prospectId) : fetch les champs utilisés par
...
export interface ProspectForPartenaire {
  id: string;
  status: string;
...
export async function getProspectForPartenaire(
  prospectId: string,
): Promise<ProspectForPartenaire | null> {
  const supabase = getSupabaseServiceClient();
  const { data, error } = await supabase
    .from('prospects')
    .select(
