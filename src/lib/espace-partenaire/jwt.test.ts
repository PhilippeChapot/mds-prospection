 ...
 *
 * P5.x.2 — tests JWT helpers Espace Partenaire.
 *
 * Couvre :
...
describe('Espace Partenaire JWT (P5.x.2)', () => {
  ...
  it('verifyMagicToken refuse un token type=session (wrong-type)', async () => {
    const { signSessionToken, verifyMagicToken, EspacePartenaireTokenError } =
      await import('./jwt');
    const sessionToken = await signSessionToken('prospect-1');
    await expect(verifyMagicToken(sessionToken)).rejects.toMatchObject({
      name: 'EspacePartenaireTokenError',
      code: 'wrong-type',
    });
    void EspacePartenaireTokenError;
  });
