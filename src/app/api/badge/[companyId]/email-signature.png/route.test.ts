 ...
 *
 * P5.x.19 — smoke tests GET /api/badge/[id]/email-signature.png.
 *
 ...
 *   - construit bien un ImageResponse 600x120 pour un MDS standard
...
describe('GET /api/badge/[companyId]/email-signature.png (P5.x.19)', () => {
  it('404 quand la company est introuvable', async () => {
    fakeCompany = null;
    const { GET } = await import('./route');
    const res = await GET(new Request('http://localhost/api/badge/xxx/email-signature.png'), {
  ...
  it('200 + image/png pour une MDS standard sans logo (fallback nom)', async () => {
    ...
    const { GET } = await import('./route');
    const res = await GET(new Request('http://localhost/api/badge/cmp-1/email-signature.png'), {
    ...
    expect(opts).toMatchObject({ width: 600, height: 120 });
    expect((opts as { headers: Record<string, string> }).headers['Content-Disposition']).toContain(
      'email-signature-mds-2026-acme-media.png',
    );
  });
  ...
  it('200 pour un PRS exhibitor (rendu inclut le 2eme logo)', async () => {
    ...
    const { GET } = await import('./route');
    const res = await GET(new Request('http://localhost/api/badge/cmp-2/email-signature.png'), {
