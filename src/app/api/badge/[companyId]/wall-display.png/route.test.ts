 ...
 *
 * P5.x.19 — smoke tests GET /api/badge/[id]/wall-display.png.
 *
...
describe('GET /api/badge/[companyId]/wall-display.png (P5.x.19)', () => {
  it('404 quand la company est introuvable', async () => {
    fakeCompany = null;
    const { GET } = await import('./route');
    const res = await GET(new Request('http://localhost/api/badge/xxx/wall-display.png'), {
  ...
  it('200 + image/png pour une MDS standard sans logo (fallback nom)', async () => {
    ...
    const { GET } = await import('./route');
    const res = await GET(new Request('http://localhost/api/badge/cmp-1/wall-display.png'), {
    ...
    expect((opts as { headers: Record<string, string> }).headers['Content-Disposition']).toContain(
      'wall-display-mds-2026-acme-media.png',
    );
  });
  ...
  it('200 pour un PRS exhibitor (rendu inclut le 2eme logo)', async () => {
    ...
    const { GET } = await import('./route');
    const res = await GET(new Request('http://localhost/api/badge/cmp-2/wall-display.png'), {
