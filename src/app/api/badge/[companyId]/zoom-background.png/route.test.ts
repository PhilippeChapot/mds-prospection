 ...
 *
 * P5.x.19 — smoke tests GET /api/badge/[id]/zoom-background.png.
 *
 ...
 *   - construit bien un ImageResponse 1920x1080 pour un MDS standard
...
describe('GET /api/badge/[companyId]/zoom-background.png (P5.x.19)', () => {
  it('404 quand la company est introuvable', async () => {
    fakeCompany = null;
    const { GET } = await import('./route');
    const res = await GET(new Request('http://localhost/api/badge/xxx/zoom-background.png'), {
  ...
  it('200 + image/png pour une MDS standard sans logo (fallback nom)', async () => {
    ...
    const { GET } = await import('./route');
    const res = await GET(new Request('http://localhost/api/badge/cmp-1/zoom-background.png'), {
    ...
    expect((opts as { headers: Record<string, string> }).headers['Content-Disposition']).toContain(
      'zoom-background-mds-2026-acme-media.png',
    );
  });
  ...
  it('200 pour un PRS partner (rendu inclut le 2eme logo)', async () => {
    ...
    const { GET } = await import('./route');
    const res = await GET(new Request('http://localhost/api/badge/cmp-2/zoom-background.png'), {
