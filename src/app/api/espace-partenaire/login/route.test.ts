 ...
 *
 * P5.x.2.bis — tests GET /api/espace-partenaire/login.
 *
 * Cas couverts :
...
describe('GET /api/espace-partenaire/login (P5.x.2.bis)', () => {
  ...
  it('token magic valide -> 307 redirect /fr/dashboard + cookie session', async () => {
    const { signMagicToken } = await import('@/lib/espace-partenaire/jwt');
    const token = await signMagicToken('prospect-123');

    const { GET } = await import('./route');
    const req = new NextRequest(
      new URL(`http://localhost/api/espace-partenaire/login?token=${token}&locale=fr`),
    );
    const res = await GET(req);

    expect(res.status).toBe(307);
    expect(res.headers.get('location')).toContain('/fr/espace-partenaire/dashboard');
    const setCookie = res.headers.get('set-cookie');
    expect(setCookie).toContain('espace_partenaire_session=');
  ...
  it('locale=en -> redirect /en/dashboard', async () => {
    ...
    const { GET } = await import('./route');
    const req = new NextRequest(
      new URL(`http://localhost/api/espace-partenaire/login?token=${token}&locale=en`),
    );
    const res = await GET(req);

    expect(res.status).toBe(307);
    expect(res.headers.get('location')).toContain('/en/espace-partenaire/dashboard');
  });

  it('token absent -> redirect ?error=invalid', async () => {
    const { GET } = await import('./route');
    const req = new NextRequest(new URL('http://localhost/api/espace-partenaire/login'));
    const res = await GET(req);

    expect(res.status).toBe(307);
    expect(res.headers.get('location')).toMatch(/\/fr\/espace-partenaire\?error=invalid/);
  });
  ...
  it('token expire -> redirect ?error=expired', async () => {
    ...
    const { GET } = await import('./route');
    const req = new NextRequest(
      new URL(`http://localhost/api/espace-partenaire/login?token=${expiredToken}&locale=fr`),
    );
    const res = await GET(req);

    expect(res.status).toBe(307);
    expect(res.headers.get('location')).toMatch(/\/fr\/espace-partenaire\?error=expired/);
  });
  ...
  it('token tampered -> redirect ?error=invalid', async () => {
    ...
    const { GET } = await import('./route');
    const req = new NextRequest(
      new URL(`http://localhost/api/espace-partenaire/login?token=${tampered}`),
    );
    const res = await GET(req);
  ...
  it('token type=session refuse comme magic -> redirect ?error=invalid (wrong-type)', async () => {
    const { signSessionToken } = await import('@/lib/espace-partenaire/jwt');
    const sessionToken = await signSessionToken('p1');

    const { GET } = await import('./route');
    const req = new NextRequest(
      new URL(`http://localhost/api/espace-partenaire/login?token=${sessionToken}`),
    );
    const res = await GET(req);
