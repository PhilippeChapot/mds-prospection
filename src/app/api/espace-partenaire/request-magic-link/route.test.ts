 ...
 *
 * P5.x.2 — tests POST /api/espace-partenaire/request-magic-link.
 *
 * Cas couverts :
...
describe('POST /api/espace-partenaire/request-magic-link (P5.x.2)', () => {
  ...
  it('email match prospect actif -> envoi Resend + 200 success', async () => {
    ...
    expect(sendArgs.subject).toContain('Espace Partenaire');
    // P5.x.2.bis : URL pointe maintenant vers le Route Handler /api/...
    // (le `&` est echappe en `&amp;` dans le HTML).
    expect(sendArgs.html).toContain('test.mediadays.solutions/api/espace-partenaire/login?token=');
    expect(sendArgs.html).toContain('locale=fr');
    expect(sendArgs.text).toContain('test.mediadays.solutions/api/espace-partenaire/login?token=');
  });
