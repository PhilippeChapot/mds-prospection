/**
 * POST /api/espace-exposant/request-magic-link — P5.x.2
 *
 * Demande d'envoi d'un lien magic-link a l'email d'un exposant deja
...
const LOG_PREFIX = '[espace-exposant/request-magic-link]';
...
export async function POST(request: Request) {
  ...
  const ipLimit = checkRateLimit({
    key: `espace-exposant-magic:ip:${ip}`,
    limit: 10,
    windowSeconds: 60 * 60,
  });
  if (!ipLimit.ok) {
  ...
  const emailLimit = checkRateLimit({
    key: `espace-exposant-magic:email:${email}`,
    limit: 5,
    windowSeconds: 60 * 60,
  });
  if (!emailLimit.ok) {
  ...
  try {
    ...
    await sendTransactionalEmailViaResend({
      ...
      text: tpl.text,
      tags: [
        { name: 'category', value: 'espace_exposant_magic_link' },
