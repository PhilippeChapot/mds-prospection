...
import {
  ...
  type ExhibitorResourceRow,
} from '@/lib/exhibitor-resources/actions';
...
export function ExhibitorResourcesClient({ resources }: { resources: ExhibitorResourceRow[] }) {
  const router = useRouter();
  const [search, setSearch] = useState('');
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [form, setForm] = useState<FormState>(EMPTY_FORM);
  const [deleteTarget, setDeleteTarget] = useState<ExhibitorResourceRow | null>(null);
  ...
  function openEdit(row: ExhibitorResourceRow) {
    setForm({
      id: row.id,
      slug: row.slug,
            ...
            <aside className="border-md-border bg-md-bg-soft/40 w-64 shrink-0 space-y-4 border-r p-5">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-md-text-muted text-[11px]">
                  Tri ascendant côté exposant (10, 20, 30…).
                </p>
              </div>
              ...
              <div className="space-y-1.5">
                <Label className="flex cursor-pointer items-center gap-2">
                  <input
                    ...
                    className="size-4"
                  />
                  <span>Publiée (visible exposants)</span>
                </Label>
              </div>
