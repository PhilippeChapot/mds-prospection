...
vi.mock('@/lib/espace-exposant/jwt', () => ({
  ...
  EspaceExposantTokenError: class extends Error {},
}));
...
describe('updateCompanySlugAction (P5.x.16-bis)', () => {
  ...
  it('happy path -> ok + UPDATE + revalidatePath FR + EN', async () => {
    ...
    expect(revalidateCalls).toContain('/fr/espace-exposant/dashboard');
    expect(revalidateCalls).toContain('/en/espace-exposant/dashboard');
  });
});
