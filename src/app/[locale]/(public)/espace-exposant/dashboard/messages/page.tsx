...
import { NewExposantConversationButton } from './NewExposantConversationButton';
...
/**
 * P9.2 — page messagerie pour l'exposant connecte.
 *
 * Liste les conversations support de cet exposant (les seules visibles
 ...
 * vers staff_pool.
 */
export default async function EspaceExposantMessagesPage({ params }: PageProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex justify-end">
        <NewExposantConversationButton locale={locale === 'en' ? 'en' : 'fr'} />
      </div>
      ...
      {conversations.length === 0 ? (
        ...
        <ul className="border-md-border bg-card divide-md-border divide-y rounded-xl border shadow-sm">
          {conversations.map((c) => (
            <li key={c.id}>
              <Link
                href={`/${locale}/espace-exposant/dashboard/messages/${c.id}`}
