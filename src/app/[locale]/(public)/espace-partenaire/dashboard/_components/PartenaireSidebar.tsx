...
import { getSpaceTitle, type ContactProfile } from '@/lib/espace-partenaire/detect-profile';

/**
 * P5.x.17 — Sidebar de l'Espace Partenaire V1.3.
 *
 ...
 *   - PartenaireMobileMenu : dans le drawer (Sheet) mobile
 *
...
export function PartenaireSidebar({ onNavigate, profile }: Props) {
  const pathname = usePathname() ?? '';
  const locale = useLocale();
  const t = useTranslations('espacePartenaire.nav');

  const baseHref = `/${locale}/espace-partenaire/dashboard`;
