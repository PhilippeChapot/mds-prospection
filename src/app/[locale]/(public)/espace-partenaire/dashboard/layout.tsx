 ...
 *     contactId (et prospectId si dispo). Redirect vers /espace-partenaire
 ...
 *     qui pilotent le menu dynamique (is_partenaire, is_lead, is_affiliate,
 *     has_stand).
 *   - Shell : sidebar desktop fixe 240px + header mobile burger.
 *   - Titre adaptatif selon profil : "Espace partenaire" / "Espace affilié"
...
export default async function EspacePartenaireDashboardLayout({ children, params }: LayoutProps) {
  ...
  // etre coherent avec PartenaireSidebar h2 + SheetTitle sr-only mobile.
    ...
    <div className="bg-md-bg flex min-h-svh flex-col">
      ...
      <div className="flex flex-1">
        ...
        <aside className="border-md-border bg-card hidden w-60 shrink-0 border-r md:block">
          <PartenaireSidebar profile={profile} />
        </aside>
