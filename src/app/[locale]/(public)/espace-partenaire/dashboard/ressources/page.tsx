...
import { getPublishedResourcesAction } from '@/lib/partner-resources/actions';

export const dynamic = 'force-dynamic';
export const metadata = { title: 'Ressources — Espace Partenaire' };
...
export default async function RessourcesPage({ params }: PageProps) {
  ...
  const t = await getTranslations({ locale, namespace: 'PartnerResources' });
      ...
      {resources.length === 0 ? (
        ...
        <ul className="space-y-2">
          {resources.map((r) => (
            <li key={r.id}>
              <Link
                href={`/${locale}/espace-partenaire/dashboard/ressources/${r.slug}`}
