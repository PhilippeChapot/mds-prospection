...
import { listSupplementaryOrdersForProspect } from '@/lib/espace-partenaire/supplementary-orders/queries';
import { formatEurHt } from '@/lib/tarifs/format';
import { formatParisDate } from '@/lib/format/dates';

export const dynamic = 'force-dynamic';
export const metadata = { title: 'Mes commandes complémentaires — Espace Partenaire' };
...
function fmtDate(iso: string): string {
  try {
    ...
    return formatParisDate(iso, { day: '2-digit', month: 'short', year: 'numeric' });
  } catch {
    ...
    <div className="space-y-5">
      ...
      {orders.length === 0 ? (
        <div className="bg-card border-md-border text-md-text-muted rounded-xl border p-12 text-center text-sm shadow-sm">
          ...
          <Link
            href={`/${locale}/espace-partenaire/dashboard/commander`}
              ...
              {orders.map((o) => {
                ...
                return (
                  <tr key={o.id} className="border-md-border hover:bg-muted/30 border-t">
                    ...
                    <td className="px-4 py-3">
                      <Link
                        href={`/${locale}/espace-partenaire/dashboard/commandes/${o.id}`}
