...
import { requestPartnerPasswordResetAction } from '@/lib/auth/partner-password-reset-actions';

type Status = 'idle' | 'submitting' | 'success' | 'error';

export function ForgotPasswordForm({ locale }: { locale: 'fr' | 'en' }) {
  const t = useTranslations('espacePartenaire.forgotPassword');
    ...
    <form onSubmit={onSubmit} noValidate>
      <Card className="border-md-border space-y-5 p-5 shadow-sm sm:p-7">
        <div className="space-y-1.5">
          <Label htmlFor="forgot-email" className="font-semibold">
            {t('emailLabel')}
          </Label>
          <Input
            id="forgot-email"
