...
export async function generateMetadata({
  ...
  return {
    title:
      locale === 'en'
        ? 'Reset password · Partner portal'
        : 'Réinitialiser le mot de passe · Espace Partenaire',
  };
}
...
export default async function ResetPasswordPage({ params, searchParams }: PageProps) {
  ...
  const t = await getTranslations({ locale, namespace: 'espacePartenaire.resetPassword' });
    ...
    <section className="mx-auto max-w-xl px-4 py-10 sm:px-6 sm:py-16">
      <div className="mb-8 text-center">
        <p className="text-md-magenta mb-2 text-xs font-semibold tracking-widest uppercase">
          {safeLocale === 'en' ? 'Partner portal' : 'Espace Partenaire'}
        </p>
