...
import { consumePartnerPasswordResetAction } from '@/lib/auth/partner-password-reset-actions';

type Status = 'idle' | 'submitting' | 'success' | 'error';

export function ResetPasswordForm({ token }: { token: string }) {
  const t = useTranslations('espacePartenaire.resetPassword');
      ...
      <Card className="border-md-border space-y-5 p-5 shadow-sm sm:p-7">
        <div className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="reset-new-pwd" className="font-semibold">
              {t('newPasswordLabel')}
            </Label>
            <Input
              id="reset-new-pwd"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="reset-confirm-pwd" className="font-semibold">
              {t('confirmLabel')}
            </Label>
            <Input
